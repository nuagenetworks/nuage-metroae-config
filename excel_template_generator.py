...
class ExcelTemplateGenerator(object):
    ...
    def write_workbook(self, output_file, example_dir=None):
        ...
        for schema_name in schema_names:
            ...
            if first and self.settings["show_version"]:
                self.write_version(workbook.active)
            first = False
    ...
    def write_version(self, worksheet):
        worksheet[self.settings["version_coords"]] = ("v" +
    ...
    def read_schema(self, schema_name):
        if self.settings["schemas"] is not None:
            if schema_name in self.settings["schemas"]:
                schema_str = self.settings["schemas"][schema_name]
            else:
                raise Exception("No schema %s defined in schema settings" %
                                schema_name)
        else:
