...
SERVICE_CHAINING_POLICY_KEY = \
                            TypeToObjectName("action",
                                             {"REDIRECT":
                                               "redirection_target_name",
                                              "FORWARD":
                                               "ingress_forwarding_policy_name"
                                              })
...
INGRESS_DESTINATION_LOCATION_TYPE = \
                                  TypeToObjectName("destination_location_type",
                                                   COMMON_LOCATION_TYPE)

EGRESS_SOURCE_LOCATION_TYPE = TypeToObjectName("source_location_type",
                                               COMMON_LOCATION_TYPE)
EGRESS_DESTINATION_LOCATION_TYPE = \
                                  TypeToObjectName("destination_location_type",
                                                   COMMON_LOCATION_TYPE)

SERVICE_GROUP_TYPE = \
                 TypeToObjectName("l4_service_or_group_type",
...
LIST_DEPENDENCY_KEYS = {"monitor scope": {"destination_nsgs": "nsg_name",
                        ...
                        "network performance binding":
                                          ...
                                          "l2_domain_names": "l2_domain_name"},
                        "nsg access port":
                         {"egress_qos_policy_names": "egress_qos_policy_name"}}
...
REPLACEMENT_KEYS = \
    {"ingress qos policy":
     ...
     "ingress forwarding policy entry": {
              "source_location_zone_name": "zone_name",
              "destination_location_zone_name": "zone_name",
     ...
     "ingress security policy entry": {
      ...
      "virtual ip": {"port_name": [('nsg_name', 'nsg_access_port_name'),
                   ('gateway_name', 'port_name')]},
      "redirection target binding": {"port_name":
                                  [('nsg_name', 'nsg_access_port_name'),
                                   ('gateway_name', 'port_name')]},
      "policy group binding": {"port_name":
                                    [('nsg_name', 'nsg_access_port_name'),
                                    ('gateway_name', 'port_name')]},
        ...
        if version:
            if re.match('.*v[0-9].*', sorted_file_names[i]):
                if version == sorted_file_names[i].split('_')[-1][1:]:
                    if sorted_file_names[i].startswith(
                                                sorted_file_names[i - 1][:-4]):
                        filtered_file_list.pop()

                    filtered_file_list.append(sorted_file_names[i])
            elif sorted_file_names[i + 1].startswith(
                                                sorted_file_names[i][:-4]):
                filtered_file_list.append(sorted_file_names[i])
        else:
            if not re.match('.*v[0-9].*', sorted_file_names[i]):
                filtered_file_list.append(sorted_file_names[i])
            else:
                if not sorted_file_names[i].startswith(
                                                sorted_file_names[i - 1][:-4]):
                    filtered_file_list.append(sorted_file_names[i])
...
def load_data(args, jinja2_template_data):
    ...
    for file_name in filter_data_file(sorted(os.listdir(args.data_path)),
                                     args.version):
        udp = UserDataParser()
        ...
        for user_data in udp.data:
            ...
            template_object_name = get_object_name(user_data[1],
                                        jinja2_template_data[template_name])
            ...
            elif args.group in ["vnsaar", "vnswifi", "network"] and \
                   template_name == "nsg network port":
                object_name_to_data = group_user_data.get(template_name, {})
                object_name_to_data[template_object_name] = user_data
                group_user_data[template_name] = object_name_to_data
            elif args.group in ["security", "advrouting"] and \
                  template_name in ["bridge port", "application binding",
                                 "monitor scope",
                                 "application performance management binding",
...
def calculate_template_dependencies(template_dict, group_user_data):
    ...
    for key in group_user_data:
        ...
        for variable in template_variables:
            ...
            if variable_type == "reference" and \
                     variable["name"] not in execlude:
...
def resolve_list_dependencies(template_name,
    ...
    for value in curr_object_data[dependency]:
        ...
        if not found:
            found, data, temp_template_name, temp_key = \
                                                    find_dependency(tuple_key,
                                                        extra_keys_dict)
...
def get_replacement_keys(template_name, dependency, curr_object_data):
    ...
    if template_name in REPLACEMENT_KEYS and \
        ...
        tuple_key = (REPLACEMENT_KEYS[template_name][dependency],
                    curr_object_data[dependency])

        if isinstance(REPLACEMENT_KEYS[template_name][dependency],
                      TypeToObjectName):
        ...
        elif isinstance(REPLACEMENT_KEYS[template_name][dependency],
                      Expression):
...
def resolve_single_dependencies(template_name,
    ...
    tuple_keys = replacement_tuple_keys \
                        if len(replacement_tuple_keys) > 0 else tuple_keys
                ...
                if dependency in object_data[1]:
                    if template_name in LIST_DEPENDENCY_KEYS and\
                        ...
                        dependencies_not_found.extend(
                            resolve_list_dependencies(template_name,
                                                      dependency,
                                                      group_user_data,
                                                      extra_keys_dict,
                                                      object_data[1]))
                        ...
                        dependencies_not_found.extend(
                              resolve_single_dependencies(template_name,
                                                    dependency,
                                                    group_user_data,
                                                    extra_keys_dict,
                                                    object_data[1]))
